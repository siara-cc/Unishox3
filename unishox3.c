...
byte usx_hcodes[] = {0, 0, 0, 0, 0};
char usx_hcode_count = 1;
...
/// Code for special code (111) when state=USX_DELTA
#define UNI_STATE_SPL_CODE 0xE3
...
/// Special code to add hcode
#define ADD_HCODE 0x40
/// Length of Add hcode
#define ADD_HCODE_LEN 3
...
void reset_hcodes() {
  //memset(usx_hcodes, '\0', sizeof(usx_hcodes));
  //usx_hcode_count = 1;
        usx_hcodes[USX_ALPHA] = 0x02; // 00
      usx_hcodes[USX_SYM]   = 0x42; // 01
      usx_hcodes[USX_NUM]   = 0x82; // 10
      usx_hcodes[USX_DICT]  = 0xC3; // 110
      usx_hcodes[USX_DELTA] = 0xE3; // 111
      usx_hcode_count = 5;
}
...
int add_hcode(char *out, int olen, int ol, int set) {
  if (usx_hcodes[set])
    return ol;
  switch (usx_hcode_count) {
    case 1:
      usx_hcodes[USX_ALPHA] = 0x01; // 0
      usx_hcodes[set] = 0x81;       // 1
      usx_hcode_count += (set << 3);
      break;
    case 2:
      usx_hcodes[USX_ALPHA] = 0x02; // 00
      usx_hcodes[set]       = 0x42; // 01
      usx_hcode_count += (set << 6);
      break;
    case 3:
      usx_hcodes[USX_ALPHA]                     = 0x02; // 00
      usx_hcodes[set]                           = 0xC2; // 11
      usx_hcodes[(usx_hcode_count >> 3) & 0x03] = 0x82; // 10
      break;
    case 4:
      usx_hcodes[USX_ALPHA] = 0x02; // 00
      usx_hcodes[USX_SYM]   = 0x42; // 01
  ...
  usx_hcode_count++;
  SAFE_APPEND_BITS(ol = append_bits(out, olen, ol, ADD_HCODE, ADD_HCODE_LEN));
  SAFE_APPEND_BITS(ol = append_bits(out, olen, ol, (set - 1) << 6, 2));
  return ol;
}
...
int append_code(char *out, int olen, int ol, byte code, byte *state) {
  ...
  if (hcode == USX_SYM || *state != hcode) {
    SAFE_APPEND_BITS(ol = append_switch_code(out, olen, ol, *state));
    if (usx_hcodes[hcode]) {
      SAFE_APPEND_BITS(ol = append_bits(out, olen, ol, usx_hcodes[hcode] & 0xF8, usx_hcodes[hcode] & 0x03));
    } else {
      add_hcode(out, olen, ol, hcode);
    }
    if (*state != USX_ALPHA && hcode == USX_ALPHA)
      *state = USX_ALPHA;
    if (*state != USX_NUM && hcode == USX_NUM && usx_sets[hcode][vcode] >= '0' && usx_sets[hcode][vcode] <= '9')
      *state = USX_NUM;
  }
...
const byte count_codes[] = {0x02, 0x42, 0x82, 0xC3, 0xE3};
...
int encodeUnicode(char *out, int olen, int ol, int32_t code, int32_t prev_code) {
  ...
  const byte codes[6] = {0x02, 0x42, 0x83, 0xA3, 0xC3, 0xE3};
  int32_t till = 0;
...
int matchOccurance(const char *in, int len, int l, char *out, int olen, int *ol, byte *state) {
  ...
  if (longest_len) {
    SAFE_APPEND_BITS(*ol = append_switch_code(out, olen, *ol, *state));
    if (usx_hcodes[USX_DICT])
      SAFE_APPEND_BITS(*ol = append_bits(out, olen, *ol, usx_hcodes[USX_DICT] & 0xF8, usx_hcodes[USX_DICT] & 0x03));
    else
      add_hcode(out, olen, *ol, USX_DICT);
...
/// This is a crude implementation that is not optimized.  Assuming only short strings \n
/// are encoded, this is not much of an issue.
int matchLine(const char *in, int len, int l, char *out, int olen, int *ol, struct us_lnk_lst *prev_lines, byte *state) {
  int last_ol = *ol;
  int last_len = 0;
  int last_dist = 0;
  int last_ctx = 0;
...
int append_nibble_escape(char *out, int olen, int ol, byte state) {
  SAFE_APPEND_BITS(ol = append_switch_code(out, olen, ol, state));
  if (usx_hcodes[USX_DICT])
    SAFE_APPEND_BITS(ol = append_bits(out, olen, ol, usx_hcodes[USX_NUM] & 0xF8, usx_hcodes[USX_NUM] & 0x03));
  else
    add_hcode(out, olen, ol, USX_NUM);
...
int unishox3_compress_lines(const char *in, int len, UNISHOX_API_OUT_AND_LEN(char *out, int olen), const char *usx_freq_seq[], const char *usx_templates[], struct us_lnk_lst *prev_lines) {
  ...
  for (l=0; l<len; l++) {
    ...
    if (l < (len - NICE_LEN + 1)) {
      if (prev_lines) {
        l = matchLine(in, len, l, out, olen, &ol, prev_lines, &state);
        if (l > 0) {
          continue;
        } else if (l < 0 && ol < 0) {
          return olen + 1;
        ...
        l = -l;
          ...
          l = matchOccurance(in, len, l, out, olen, &ol, &state);
          if (l > 0) {
            continue;
          } else if (l < 0 && ol < 0) {
            return olen + 1;
      ...
      for (i = 0; i < 6; i++) {
        ...
        if (len - seq_len >= 0 && l <= len - seq_len) {
          if (memcmp(usx_freq_seq[i], in + l, seq_len) == 0) {
            SAFE_APPEND_BITS2(rawolen, ol = append_code(out, olen, ol, usx_freq_codes[i], &state));
            l += seq_len;
            l--;
            break;
          }
    ...
    if (c_in >= 'A' && c_in <= 'Z')
      ...
      if (is_all_upper) {
        ...
        SAFE_APPEND_BITS2(rawolen, ol = append_bits(out, olen, ol, usx_hcodes[USX_ALPHA] & 0xF8, usx_hcodes[USX_ALPHA] & 0x03));
        state = USX_ALPHA;
      }
    }
      ...
      if (uni) {
        ...
        if (state != USX_DELTA) {
          ...
          if (uni2) {
            ...
            SAFE_APPEND_BITS2(rawolen, ol = append_switch_code(out, olen, ol, state));
            SAFE_APPEND_BITS2(rawolen, ol = append_bits(out, olen, ol, usx_hcodes[USX_DELTA] & 0xF8, usx_hcodes[USX_DELTA] & 0x03));
            SAFE_APPEND_BITS2(rawolen, ol = append_bits(out, olen, ol, UNI_STATE_SPL_CODE, UNI_STATE_SPL_CODE_LEN));
  ...
  if (need_full_term_codes) {
    ...
    append_final_bits(out, rst, ol, state, is_all_upper);
    return rst;
  }
}

// Main API function. See unishox2.h for documentation
int unishox3_compress(const char *in, int len, UNISHOX_API_OUT_AND_LEN(char *out, int olen), const char *usx_freq_seq[], const char *usx_templates[]) {
  return unishox3_compress_lines(in, len, UNISHOX_API_OUT_AND_LEN(out, olen), usx_freq_seq, usx_templates, NULL);
}

// Main API function. See unishox2.h for documentation
int unishox3_compress_simple(const char *in, int len, char *out) {
  return unishox3_compress_lines(in, len, UNISHOX_API_OUT_AND_LEN(out, INT_MAX - 1), USX_FREQ_SEQ_DFLT, USX_TEMPLATES, NULL);
}
...
int readHCodeIdx(const char *in, int len, int *bit_no_p) {
  if (!usx_hcodes[USX_ALPHA])
    return USX_ALPHA;
  if (*bit_no_p < len) {
    byte code = read8bitCode(in, len, *bit_no_p);
    for (int code_pos = 0; code_pos < 5; code_pos++) {
      if ((usx_hcodes[code_pos] & 0xF8) && (code & len_masks[(usx_hcodes[code_pos] & 0x03) - 1]) == (usx_hcodes[code_pos] & 0xF8)) {
        *bit_no_p += usx_hcodes[code_pos] & 0x03;
        return code_pos;
      }
    }
  }
...
int32_t readUnicode(const char *in, int *bit_no_p, int len) {
  int idx = readCode(in, bit_no_p, len, 6);
  if (idx == 99)
...
int decodeRepeat(const char *in, int len, char *out, int olen, int ol, int *bit_no, struct us_lnk_lst *prev_lines) {
  if (prev_lines) {
    ...
    memcpy(out + ol, cur_line->data + dist, min_of(left, dict_len));
...
// Main API function. See unishox2.h for documentation
int unishox3_decompress_lines(const char *in, int len, UNISHOX_API_OUT_AND_LEN(char *out, int olen), const char *usx_freq_seq[], const char *usx_templates[], struct us_lnk_lst *prev_lines) {
  ...
  while (bit_no < len) {
    ...
    if (dstate == USX_DELTA || h == USX_DELTA) {
      ...
      if ((delta >> 8) == 0x7FFFFF) {
        ...
        spl_code_idx = getStepCodeIdx(in, len, &bit_no, 4);
        switch (spl_code_idx) {
          case 0:
    ...
    if (v == 0 && h != USX_SYM) {
      ...
      if (h == USX_ALPHA) {
         if (dstate == USX_ALPHA) {
           if (!(usx_hcodes[USX_ALPHA] & 0x03) && TERM_BYTE_PRESET_1 == (read8bitCode(in, len, bit_no - SW_CODE_LEN) & (0xFF << (8 - (is_all_upper ? TERM_BYTE_PRESET_1_LEN_UPPER : TERM_BYTE_PRESET_1_LEN_LOWER)))))
...
// Main API function. See unishox2.h for documentation
int unishox3_decompress(const char *in, int len, UNISHOX_API_OUT_AND_LEN(char *out, int olen), const char *usx_freq_seq[], const char *usx_templates[]) {
  return unishox3_decompress_lines(in, len, UNISHOX_API_OUT_AND_LEN(out, olen), usx_freq_seq, usx_templates, NULL);
}

// Main API function. See unishox2.h for documentation
int unishox3_decompress_simple(const char *in, int len, char *out) {
  return unishox3_decompress(in, len, UNISHOX_API_OUT_AND_LEN(out, INT_MAX - 1), USX_FREQ_SEQ_DFLT, USX_TEMPLATES);
}
